...
public class RoomCamera : MonoBehaviour
{
    ...
    public void MoveToRoom(BoxCollider2D roomCollider, float roomCountdownDuration) // YENİ PARAMETRE EKLENDİ
    {
        currentRoomBounds = roomCollider;

        // KRİTİK: Yeni odanın geri sayımını başlat
        if (GameManager.Instance != null)
        {
            GameManager.Instance.StartCountdownForRoom(roomCountdownDuration); // roomCountdownDuration > 0 OLMALI
        }
        else
        {
            Debug.LogError("GameManager.Instance null! Sayaç başlatılamadı.");
        }
    }
